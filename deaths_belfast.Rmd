...
Victims by year and location
```{r}
mc_clean %>%
  filter(location < 19) %>%
  mutate(total = nrow(.)) #3390 deaths in NI 

mc_clean %>%
  filter(location >= 19) %>%
  mutate(total = nrow(.)) #259 deaths outside NI
 
```

All deaths in NI, by year
```{r}
mc_clean %>%
  filter(location < 19) %>%
  ...
  arrange(year) %>%
  ungroup () %>%
  mutate(location_name = recode(location_name, 'West Belfast' = 'Belfast West', 'North Belfast' = 'Belfast North' , 'South Belfast' = 'Belfast South', 'East Belfast' = 'Belfast East')) #%>%
